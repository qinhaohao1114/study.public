...
public class MasterWirteStateMaster implements MasterHandlerState {
    @Override
    public void changeState(TCPMasterHandler h) {
        h.setState(new MasterReadStateMaster());
    }
    ...
    public void handle(TCPMasterHandler h, SelectionKey sk, SocketChannel sc, ThreadPoolExecutor pool) throws IOException {
        ...
        h.setState(new MasterReadStateMaster()); // 改變狀態(SENDING->READING)
